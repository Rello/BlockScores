...
contract BlockScores {
    ...
    */
    ...
    function addNewBoard(bytes32 name, string boardDescription) public payable returns(bytes32 boardHash){
        ...
        balance += msg.value;
        boardHash = keccak256(name, msg.sender);
        numBoards++;
    ...
    /// @notice Overwrite board metadata as contract owner only
    ...
    /// @return true
    function setBoardMetadata(bytes32 boardHash, bytes32 name, string boardDescription, uint8 numPlayers, address boardOwner) isOwner public returns(bool) {
